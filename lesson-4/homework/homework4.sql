...
-- 4. Get the names of employees who have made at least one sale.
SELECT DISTINCT e.name
FROM Employees e
JOIN Sales s ON e.employee_id = s.employee_id;

-- 5. Find the total quantity sold per product.
SELECT 
    p.name,
    SUM(s.quantity) AS total_quantity_sold
FROM Sales s
JOIN Products_Discounted p ON s.product_id = p.product_id
GROUP BY p.name;

-- 6. Show the most recent sale made.
SELECT *
FROM Sales
...
-- 7. Find the employee who made the highest total sales value (after discount).
SELECT 
    e.name,
    SUM((p.price - (p.price * p.discount / 100)) * s.quantity) AS total_sales_value
FROM Sales s
...
-- 8. List products that were ordered but never sold.
SELECT DISTINCT p.name
FROM Orders o
JOIN Products_Discounted p ON o.product_id = p.product_id
WHERE o.product_id NOT IN (
    SELECT DISTINCT product_id FROM Sales
);


-- 9. Get the average discount of products sold by each department.
SELECT 
    e.department,
