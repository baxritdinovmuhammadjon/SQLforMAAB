...
-- Task 1: Products with discounts and price over 1000
SELECT p.*
FROM Products p
JOIN Products_Discounted pd ON p.product_id = pd.product_id
WHERE p.price > 1000;

-- Task 2: Orders with customer full names
SELECT o.*, c.first_name, c.last_name
FROM Orders o
JOIN Customers c ON o.customer_id = c.customer_id;

-- Task 3: Invoices over 5000 with customer info
SELECT i.*, c.first_name, c.last_name
FROM Invoices i
JOIN Customers c ON i.customer_id = c.customer_id
WHERE i.total_amount > 5000;

-- Task 4: Products with discount over 30%
SELECT p.*, pd.discount_percentage
FROM Products p
JOIN Products_Discounted pd ON p.product_id = pd.product_id
WHERE pd.discount_percentage > 30;

-- Task 5: Cities with population over 1 million
SELECT *
FROM City_Population
WHERE population > 1000000;

-- Task 6: Orders with total price and product name
SELECT o.order_id, o.order_date, p.product_name, o.quantity, (o.quantity * p.price) AS total_price
FROM Orders o
JOIN Products p ON o.product_id = p.product_id;

-- Task 7: Customers who ordered discounted products
SELECT DISTINCT c.customer_id, c.first_name, c.last_name
FROM Customers c
JOIN Orders o ON c.customer_id = o.customer_id
JOIN Products_Discounted pd ON o.product_id = pd.product_id;

-- Task 8: Invoices with product names
SELECT i.invoice_id, i.total_amount, p.product_name
FROM Invoices i
JOIN Orders o ON i.order_id = o.order_id
JOIN Products p ON o.product_id = p.product_id;

-- Task 9: Products not discounted
SELECT *
FROM Products
WHERE product_id NOT IN (SELECT product_id FROM Products_Discounted);

-- Task 10: Customers from cities with population > 1 million
SELECT c.*
FROM Customers c
JOIN City_Population cp ON c.city = cp.city
WHERE cp.population > 1000000;

-- Task 11: Total orders per customer
SELECT c.customer_id, c.first_name, c.last_name, COUNT(o.order_id) AS total_orders
FROM Customers c
JOIN Orders o ON c.customer_id = o.customer_id
GROUP BY c.customer_id, c.first_name, c.last_name;

-- Task 12: Products sold more than 100 times
...
-- Task 13: Invoices with customer names and city
SELECT i.*, c.first_name, c.last_name, c.city
FROM Invoices i
JOIN Customers c ON i.customer_id = c.customer_id;

-- Task 14: Customers who never ordered
SELECT *
FROM Customers
WHERE customer_id NOT IN (SELECT DISTINCT customer_id FROM Orders);

-- Task 15: Average invoice total per city
SELECT c.city, AVG(i.total_amount) AS avg_invoice
FROM Customers c
JOIN Invoices i ON c.customer_id = i.customer_id
GROUP BY c.city;

-- Task 16: Top 3 cities by total sales
SELECT TOP 3 cp.city, SUM(i.total_amount) AS total_sales
FROM City_Population cp
JOIN Customers c ON cp.city = c.city
JOIN Invoices i ON c.customer_id = i.customer_id
GROUP BY cp.city
ORDER BY total_sales DESC;

-- Task 17: Products with highest discount
SELECT TOP 1 p.product_id, p.product_name, pd.discount_percentage
FROM Products p
JOIN Products_Discounted pd ON p.product_id = pd.product_id
ORDER BY pd.discount_percentage DESC;

-- Task 18: Customers who bought more than 5 different products
SELECT c.customer_id, c.first_name, c.last_name, COUNT(DISTINCT o.product_id) AS different_products
FROM Customers c
JOIN Orders o ON c.customer_id = o.customer_id
GROUP BY c.customer_id, c.first_name, c.last_name
HAVING COUNT(DISTINCT o.product_id) > 5;

-- Task 19: Total sales per product category (assuming a 'category' column exists)
SELECT p.category, SUM(o.quantity * p.price) AS total_sales
FROM Products p
JOIN Orders o ON p.product_id = o.product_id
GROUP BY p.category;

-- Task 20: Invoice details with discount applied
SELECT i.invoice_id, p.product_name, o.quantity, p.price, pd.discount_percentage,
       (o.quantity * p.price * (1 - pd.discount_percentage / 100.0)) AS discounted_total
FROM Invoices i
JOIN Orders o ON i.order_id = o.order_id
JOIN Products p ON o.product_id = p.product_id
JOIN Products_Discounted pd ON p.product_id = pd.product_id;

-- Task 21: Customers who made purchases in all months of 2024
...
-- Task 22: Products bought by each customer
SELECT c.customer_id, c.first_name, c.last_name, p.product_name
FROM Customers c
JOIN Orders o ON c.customer_id = o.customer_id
JOIN Products p ON o.product_id = p.product_id;

-- Task 23: Cities with more than 10 customers
SELECT cp.city, COUNT(c.customer_id) AS total_customers
FROM City_Population cp
JOIN Customers c ON cp.city = c.city
GROUP BY cp.city
HAVING COUNT(c.customer_id) > 10;

-- Task 24: Customers with total purchase over 10,000
SELECT c.customer_id, c.first_name, c.last_name, SUM(i.total_amount) AS total_spent
FROM Customers c
JOIN Invoices i ON c.customer_id = i.customer_id
GROUP BY c.customer_id, c.first_name, c.last_name
